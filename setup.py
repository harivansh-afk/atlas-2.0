...
def main():
    ...
    print_banner()
    print(
        "This wizard will guide you through setting up Atlas, an open-source generalist AI agent.\n"
    )
    ...
    # Check if we're in the Atlas repository
    if not check_suna_directory():
        print_error(
            "This setup script must be run from the Atlas repository root directory."
        )
        print_info("Please clone the repository first with:")
        print_info("  git clone https://github.com/Atlas-ai/Atlas.git")
        print_info("  cd Atlas")
    ...
    # Install dependencies before starting Atlas
    ...
    # Now ask how to start Atlas
    print_step(current_step, total_steps, "Starting Atlas")
