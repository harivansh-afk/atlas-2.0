...
class UpdateAgentTool(Tool):
    ...
    @xml_schema(
        ...
        mappings=[
            ...
            {
                "param_name": "agentpress_tools",
                "node_type": "element",
                "path": "agentpress_tools",
                "required": False,
            },
            {
                "param_name": "configured_mcps",
                "node_type": "element",
                "path": "configured_mcps",
                "required": False,
            },
            {
                "param_name": "avatar",
    ...
    async def get_current_agent_config(self) -> ToolResult:
        ...
        try:
            ...
            config_summary = {
                ...
                "avatar_color": agent.get("avatar_color", "#000000"),
    ...
    async def search_mcp_servers(
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                servers = sorted(
                    servers, key=lambda x: x.get("useCount", 0), reverse=True
                )[:limit]
                    ...
                    formatted_tools.append(
                        {
                            ...
                            "parameters": tool.get("inputSchema", {}).get(
                                "properties", {}
                            ),
                            "required_params": tool.get("inputSchema", {}).get(
                                "required", []
                            ),
                        }
                    )
    ...
    async def configure_mcp_server(
        ...
        try:
            ...
            update_result = (
                await client.table("agents")
                .update({"configured_mcps": current_mcps})
                .eq("agent_id", self.agent_id)
                .execute()
            )
            ...
            async with httpx.AsyncClient() as client:
                ...
                for cat in categorized:
                    categorized[cat] = sorted(
                        categorized[cat], key=lambda x: x["useCount"], reverse=True
                    )[:5]

                return self.success_response(
                    {
                        "message": f"Found popular MCP servers"
    ...
    async def test_mcp_server_connection(
        self, qualified_name: str, config: Optional[Dict[str, Any]] = None
    ) -> ToolResult:
        ...
        try:
            ...
            return self.fail_response(
                f"Failed to connect to {qualified_name}: {str(e)}"
            )
