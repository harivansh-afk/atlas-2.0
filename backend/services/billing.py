"""
Stripe Billing API implementation for Atlas on top of Basejump. ONLY HAS SUPPOT FOR USER ACCOUNTS – no team accounts. As we are using the user_id as account_id as is the case with personal accounts. In personal accounts, the account_id equals the user_id. In team accounts, the account_id is unique.
...
SUBSCRIPTION_TIERS = {
    config.STRIPE_FREE_TIER_ID: {"name": "free", "minutes": 60000},
                ...
                if is_upgrade:
                    ...
                    return {
                        ...
                        "details": {
                            ...
                            "current_price": (
                                round(current_price["unit_amount"] / 100, 2)
                                if current_price.get("unit_amount")
                                else 0
                            ),
                            "new_price": (
                                round(new_price["unit_amount"] / 100, 2)
                            ...
                            "invoice": (
                                {
                                    "id": (
                                        latest_invoice["id"] if latest_invoice else None
                                    ),
                                    "status": (
                                    ...
                                    "amount_due": (
                                        round(latest_invoice["amount_due"] / 100, 2)
                                        if latest_invoice
                                        else 0
                                    ),
                                    "amount_paid": (
                                        round(latest_invoice["amount_paid"] / 100, 2)
                                        if latest_invoice
                                        else 0
                                    ),
                                }
                    ...
                    try:
                        ...
                        sub_with_schedule = stripe.Subscription.retrieve(
                            subscription_id
                        )
                        ...
                        if schedule_id:
                            ...
                            current_phase = {
                                "items": existing_subscription["items"][
                                    "data"
                                ],  # Use original items data
                                "start_date": existing_subscription[
                                    "current_period_start"
                                ],  # Use sub start if no schedule
                        ...
                        for item in current_phase.get("items", []):
                            ...
                            if price_id and quantity is not None:
                                current_phase_items_for_api.append(
                                    {"price": price_id, "quantity": quantity}
                                )
                            else:
                                logger.warning(
                                    f"Skipping item in current phase due to missing price ID or quantity: {item}"
                                )

                        if not current_phase_items_for_api:
                            raise ValueError(
                                "Could not determine valid items for the current phase."
                            )

                        current_phase_update_data = {
                            "items": current_phase_items_for_api,
                            "start_date": current_phase[
                                "start_date"
                        ...
                        if schedule_id:
                            ...
                            logger.info(
                                f"Updating existing schedule {schedule_id} for subscription {subscription_id}"
                            )
                            logger.debug(
                                f"Current phase data: {current_phase_update_data}"
                            )
                            ...
                            logger.info(
                                f"Successfully updated schedule {updated_schedule['id']}"
                            )
                        else:
                            # Create a new schedule using the defined phases
                            print(
                                f"Creating new schedule for subscription {subscription_id}"
                            )
                            logger.info(
                                f"Creating new schedule for subscription {subscription_id}"
                            )
                            ...
                            logger.debug(
                                f"Subscription details: {subscription_id}, current_period_end_ts: {current_period_end_ts}"
                            )
                            logger.debug(
                                f"Current price: {current_price_id}, New price: {request.price_id}"
                            )
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Created new schedule {updated_schedule['id']} from subscription {subscription_id}"
                                )
                                ...
                                fetched_schedule = stripe.SubscriptionSchedule.retrieve(
                                    updated_schedule["id"]
                                )
                                logger.info(
                                    f"Schedule verification - Status: {fetched_schedule.get('status')}, Phase Count: {len(fetched_schedule.get('phases', []))}"
                                )
                                logger.debug(f"Schedule details: {fetched_schedule}")
                            except Exception as schedule_error:
                                logger.exception(
                                    f"Failed to create schedule: {str(schedule_error)}"
                                )
                        ...
                        logger.exception(
                            f"Error handling subscription schedule for sub {subscription_id}: {str(e)}"
                        )
                        raise HTTPException(
                            status_code=500,
                            detail=f"Error handling subscription schedule: {str(e)}",
                        )
            except Exception as e:
                logger.exception(
                ...
                if configurations.get("data", []):
                    ...
                    active_config = stripe.billing_portal.Configuration.update(
                        ...
                        features={
                            ...
                            "customer_update": default_config.get("features", {}).get(
                                "customer_update",
                                {
                                    "enabled": True,
                ...
                if subscription.get("status") in ["active", "trialing"]:
                    ...
                    if not has_active:
                        await client.schema("basejump").from_(
                            "billing_customers"
                        ).update({"active": False}).eq("id", customer_id).execute()
                        logger.info(
